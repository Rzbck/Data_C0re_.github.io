...
const projectMeta = {
  lumina: {
    status:'realized', type:'installation', years:'2025 2026 2027',
    poster:'assets/media/lumina/tunnel-blue.webp', video:'assets/media/lumina/experience-long.mp4?v=20260809-2'
  },
  'grand-theatre': {
    status:'realized', type:'stage', years:'2023 2024',
    poster:'assets/media/grand-theatre/hero.webp', video:'assets/media/grand-theatre/loop.mp4'
  },
  comedie: {
    status:'realized', type:'stage', years:'2021 2022 2023',
    poster:'assets/media/comedie/venue.jpg'
  },
  hardwinner: {
    status:'realized', type:'live-av stage', years:'2016 2017 2018',
    poster:'assets/media/hardwinner/lbe-2018.webp', video:'assets/media/hardwinner/amen-loop.mp4'
  },
  'stage-systems': {
    status:'realized', type:'live-av stage', years:'2016',
    poster:'assets/media/stage/funradio-wide.webp', video:'assets/media/stage/funradio-loop.mp4'
  },
  snake: {
    status:'research', type:'software interactive', years:'2026',
    poster:'assets/media/snake/gameplay.webp', video:'assets/media/snake/loop.mp4'
  },
  signal: {
    status:'simulation', type:'simulation interactive', years:'2026'
  },
  ascii: {
    status:'study', type:'study', years:'2026',
    poster:'assets/media/ascii/portrait.webp', video:'assets/media/ascii/loop.mp4'
  },
  realtime: {
    status:'research', type:'study', years:'2025 2026',
    poster:'assets/media/realtime/audio-material.webp', video:'assets/media/realtime/audio-loop.mp4'
  },
  cloud: {
    status:'study', type:'study', years:'2018',
    poster:'assets/media/cloud/pastel.webp', video:'assets/media/cloud/loop.mp4'
  }
};

const locale = {
  en:{archive:'Archive',contact:'Contact',projects:'projects',all:'All',type:'Type',year:'Year',allTypes:'All types',allYears:'All years',installation:'Installation',stage:'Stage',live:'Live AV',software:'Software',simulation:'Simulation',study:'Study',realized:'Realized',research:'R&D',empty:'No project matches these filters.'},
  fr:{archive:'Archives',contact:'Contact',projects:'projets',all:'Tous',type:'Type',year:'Année',allTypes:'Tous les types',allYears:'Toutes les années',installation:'Installation',stage:'Scène',live:'Live AV',software:'Logiciel',simulation:'Simulation',study:'Étude',realized:'Réalisé',research:'R&D',empty:'Aucun projet ne correspond à ces filtres.'},
  es:{archive:'Archivo',contact:'Contacto',projects:'proyectos',all:'Todos',type:'Tipo',year:'Año',allTypes:'Todos los tipos',allYears:'Todos los años',installation:'Instalación',stage:'Escena',live:'Live AV',software:'Software',simulation:'Simulación',study:'Estudio',realized:'Realizado',research:'I+D',empty:'Ningún proyecto coincide con estos filtros.'}
};
...
function rebuildHeader($,rel){
  ...
  const links=[
    ['archive',`${prefix}archive.html`,t.archive],
    ['cv',`${prefix}cv.html`,'CV'],
    ['contact',`${prefix}contact.html`,t.contact]
  ].map(([key,href,label])=>{
    const active=current===key;
    return `<a class="nav-text nav-primary nav-primary--${key}${active?' is-active':''}" href="${href}" data-v2-primary="${key}"${active?' aria-current="page"':''}>${label}</a>`;
  }).join('');
  const menu=actions.find('[data-menu-toggle]').first();
  if(menu.length)menu.before(links);else actions.append(links);
}

function archiveControls(lang){
  const t=locale[lang];
  const years=['2027','2026','2025','2024','2023','2022','2021','2018','2017','2016'];
  return `<section class="archive-controls" aria-label="Archive filters">
    <div>
      ...
      <div class="archive-control-row" style="margin-top:8px">
...
function enhanceArchive($,rel){
  const lang=langFor(rel),t=locale[lang];
  const shell=$('.archive-shell').first();
  if(!shell.length)return;
  shell.attr('data-archive-interactive','');
  $('.archive-controls').remove();
  $('.archive-intro').first().after(archiveControls(lang));
  $('[data-archive-empty]').remove();
  shell.after(`<p class="archive-empty" data-archive-empty>${t.empty}</p>`);

  shell.find('.archive-entry').each((_,el)=>{
    const entry=$(el),href=entry.attr('href')||'';
    const match=href.match(/projects\/([^/.]+)\.html$/);
    if(!match)return;
    const meta=projectMeta[match[1]];
    if(!meta)return;
    entry.attr('data-archive-status',meta.status).attr('data-archive-type',meta.type).attr('data-archive-years',meta.years);
    ...
    if(meta.poster){
      entry.prepend(`<span class="archive-entry-media" aria-hidden="true"><img src="${meta.poster}" alt="" loading="lazy" decoding="async">${meta.video?'<video muted loop playsinline preload="none"></video>':''}</span>`);
    }
  });

  $('script[data-archive-interactions]').remove();
  $('body').append('<script src="assets/js/archive-interactions.js" defer data-archive-interactions></script>');
}
...
console.log('Portfolio V2 final cleanup applied: persistent primary header, responsive contact, interactive archive.');
