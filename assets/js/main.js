...
(() => {
  ...
  const activateHoverPreview=row=>{
    ...
    const revealPreparedFrame=()=>{
      ...
      const reveal=()=>{if(token!==hoverPreviewToken||hoverPreviewVideo.dataset.activeSrc!==src)return;hoverPreviewStage.classList.remove('is-changing');hoverPreviewStage.classList.add('has-video');if(!motionOff)hoverPreviewVideo.play().catch(()=>{});else hoverPreviewVideo.pause()};
  ...
  const applyMotion=()=>{
    ...
    body.classList.toggle('motion-off',motionOff);pageVideos.forEach(syncVideo);
