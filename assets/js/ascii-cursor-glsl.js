(() => {
  if (window.__DATA_C0RE_ASCII_CURSOR__) return;
  window.__DATA_C0RE_ASCII_CURSOR__ = true;
  ...
  canvas.dataset.asciiCursor = 'true';
  ...
  const simulationSource = `#version 300 es
    ...
    void main(){
      ...
      if (u_inject > 0.001) {
        ...
        vec2 d = v_uv - u_mouse;
        d.x *= aspect;
        float brush = 1.0 - smoothstep(u_radius * 0.32, u_radius, length(d));
        nextB = max(nextB, brush * u_inject);
        nextA = min(nextA, 1.0 - brush * 0.28);
      }
  ...
  const displaySource = `#version 300 es
    ...
    float getCharMask(vec2 localUV, float intensity){
      ...
      if (intensity > 0.8) {
        ...
        mask = (abs(p.x) < 0.2 || abs(p.y) < 0.2) ? 1.0 : 0.0;
      } else if (intensity > 0.3) {
        mask = (abs(p.x - p.y) < 0.2 || abs(p.x + p.y) < 0.2) ? 1.0 : 0.0;
      } else if (intensity > 0.15) {
        mask = (len < 0.7 && len > 0.4) ? 1.0 : 0.0;
      } else if (intensity > 0.08) {
        mask = abs(p.y) < 0.2 ? 1.0 : 0.0;
      } else if (intensity > 0.02) {
        mask = len < 0.2 ? 1.0 : 0.0;
      }
    ...
    void main(){
      ...
      float intensity = smoothstep(0.015, 0.48, b);
      vec2 localUV = fract(fragCoord / u_cell);
      float charMask = getCharMask(localUV, intensity);
      vec3 charColor = getColor(b) * charMask;
      float alpha = charMask * smoothstep(0.02, 0.34, b) * 0.62;
  ...
  try {
    ...
    displayProgram = program(displaySource);
  } catch (error) {
    console.warn('DATA C0RE ASCII cursor disabled:', error);
    canvas.remove();
    return;
  }
  ...
  let simW = 0, simH = 0;
  let textures = [], framebuffers = [], readIndex = 0;
  ...
  const resize = () => {
    ...
    const targetW = Math.max(220, Math.min(520, Math.round(width * 0.32)));
    const targetH = Math.max(140, Math.round(targetW * height / width));
  ...
  const simulationPass = now => {
    ...
    gl.uniform2f(gl.getUniformLocation(simulationProgram, 'u_texel'), 1 / simW, 1 / simH);
    gl.uniform2f(gl.getUniformLocation(simulationProgram, 'u_mouse'), pointer.x, pointer.y);
    const active = now - pointer.lastMove < 90 ? 1 : 0;
    gl.uniform1f(gl.getUniformLocation(simulationProgram, 'u_inject'), active);
    gl.uniform1f(gl.getUniformLocation(simulationProgram, 'u_radius'), 0.026);
  ...
  const displayPass = () => {
    ...
    gl.uniform1f(gl.getUniformLocation(displayProgram, 'u_cell'), 8.0);
    gl.drawArrays(gl.TRIANGLES, 0, 6);
    gl.disable(gl.BLEND);
  };
  ...
  const wake = event => {
    ...
    idleTimer = setTimeout(() => { canvas.style.opacity = '0'; }, 4200);
    resetTimer = setTimeout(stopAndReset, 4700);
  };
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      running = false;
      cancelAnimationFrame(raf);
    } else if (performance.now() - pointer.lastMove < 4700) {
      start();
    }
  });
  reduce.addEventListener?.('change', event => {
    if (event.matches) stopAndReset();
  });
